...
```
#### set_information
Esse método é apenas um exemplo de como se registra um dado na blockchain:
...
```
Após executado, 2 arquivos com o mesmo nome do arquivo serão gerados, um com extensão ``.manifest.json`` e o outro com ``.nef``. O arquivo de extensão ``.manifest.json`` é o arquivo que que contém as assinaturas dos métodos públicos do contrato compilado, enquanto o de extensão ``.nef`` é o arquivo do contrato em bytes. Caso precise recompilar, não há necessidade de apagá-los, o compilador já sobrescreve os arquivos.
...
- Privada (localmente): <br>
  Serve para fazer teste locais e [debuggar](https://github.com/neo-project/neo-debugger) se necessário. Para debuggar o contratos compilados com o neo3-boa é necessário adicionar a [flag](https://github.com/CityOfZion/neo3-boa#configuring-the-debugger) `-d` ou `--debug`;
- TestNet:  <br>
...
> Caso queira aprender como usar o debugger da Neo, existe um [tutorial](https://ngdenterprise.com/neo-tutorials/quickstart5.html) em inglês só para isso. 
...
Caso queira desligar a rede, procure na aba `BLOCKCHAIN` a rede terminada com `.neo-express`, clique com o botão direito e clique em `Stop blockchain`. Para iniciar novamente, clique no botão `Start Neo Express` localizado na aba `QUICK START` ou procure na aba `BLOCKCHAIN` a rede terminada com `.neo-express`, clique com o botão direito e clique em `Start blockchain`.
...
Nesse tutorial iremos publicar apenas na rede privada por questões de tempo. Então tendo compilado o contrato, transfira do carteira genesis (o primeiro nó da blockchain) 100 GAS para a carteira que você ceiou. Após isso na aba `QUICK START`:
...
- Escolha de que modo chamará o contrato:
  - CalledByEntry: Esse método é para leitura e escrita e possui custo de ser usado, esse modo serve para usar apenas o contrato chamado;
  - Global: Esse método é para leitura e escrita e possui custo de ser usado, esse modo serve para usar o contrato chamado e outros contratos que este chame;
  - None: Esse modo é só para leitura da blockchain e não possui custo de ser usado;
